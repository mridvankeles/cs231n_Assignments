...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss_soft,grad = softmax_loss(scores,y)
        dx_2, dw_2, db_2 = affine_backward(grad,cache2)
        dx_1, dw_1, db_1 = affine_relu_backward(dx_2, cache1)

        #regularization
        loss = loss_soft + 0.5 * self.reg * (np.sum(self.params['W1'] * self.params['W1']) + np.sum(self.params['W2'] * self.params['W2']))
